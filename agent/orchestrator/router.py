...
_INTENT_PATTERNS: list[tuple[str, list[str]]] = [
    ...
    ("schema_sync", [
        ...
        # NOTE: トピック名はユーザーが自由に指定する任意の文字列("orders-test"
        # 等)であり、「トピック」「topic」という語すら含まないことが多い
        # (実際に "Aサイトにあるorders-testを削除して" で発生・再現した)。
        # 名前ベースのキーワード一致には限界があるため、「Aサイト/Bサイト/
        # Cサイト」という対象サイトの明示 + 作成/追加/登録/削除動詞の組み合わせを
        # 十分に強いシグナルとして扱い、フォールバックでこちらもマッチさせる。
